# https://github.com/bigcode-project/bigcode-dataset/blob/main/preprocessing/utils/text_extraction.py
...
def get_nl_ratio(text, language):
    ...
    if language == "python":
        ...
        ratio = len(comments) / len(text) if len(text) > 0 else 0
    else:
        _cs = comment_size(text, language)
        if _cs is None:
            ratio = None
        else:
            ratio = _cs / len(text) if len(text) > 0 else 0
    return ratio
